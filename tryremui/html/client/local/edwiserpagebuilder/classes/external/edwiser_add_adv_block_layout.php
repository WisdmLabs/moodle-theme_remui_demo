...
trait edwiser_add_adv_block_layout {
    ...
    public static function edwiser_add_adv_block_layout($layout, $pagetype, $region, $subpagetypepattern, $courseid, $contextinstanceid) {
        ...
        if ((version_compare($themereleaseinfo['newthemereleaseversion'], $themereleaseinfo['oldthemerelease']) > 0) && $themereleaseinfo['themeversion'] > 2023053003) {
                    ...
                    foreach ($blockcontent as $content) {
                        ...
                        if (property_exists($content, 'contenttype')) {
                            ...
                            if ($content->contenttype == 'advancedblock') {
                                ...
                                $blockrecord = $DB->get_record($blockinstancetable, array("blockname" => $blockname, "parentcontextid" => $page->context->id, "pagetypepattern" => $pagetype, "defaultregion" => $region, "defaultweight" => $count), '*');
            ...
            $responsemsg = get_string('pagelayoutaddwarningmsg', 'local_edwiserpagebuilder', $themereleaseinfo['newthemereleaseversion']);
            $responsestatus = false;
        }
